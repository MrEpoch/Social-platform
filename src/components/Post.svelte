<script lang="ts">
  ...
  function buttonEvent() {
    likeState === false ? feed.likeCount-- : feed.likeCount++;
    likeState = !likeState
  }
...
<div class="flex w-full max-w-[800px] p-6 rounded-lg dark:bg-slate-800 dark:text-white shadow bg-gray-50 flex-col gap-2">
  ...
  <div class="flex gap-4 w-fit pl-1 items-center justify-around">
    <form method="POST" use:enhance action="?/like">
      <input type="hidden" name="post_id" value={feed.id}>
      <button  type="submit" on:click={buttonEvent} class="flex gap-1">
        {#if likeState}
